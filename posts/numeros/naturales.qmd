...
```{=html}
<div style="font-family: system-ui, -apple-system, sans-serif; max-width: 620px; margin: 20px auto; padding: 25px; border: 1px solid #e2e8f0; border-radius: 12px; background: #ffffff; box-shadow: 0 10px 15px -3px rgba(0,0,0,0.08);">

  <h3 style="text-align: center; color: #1e293b; margin-top: 0; margin-bottom: 6px;">Asignación de Función $f: A \to B$</h3>
  <p style="text-align: center; color: #64748b; font-size: 14px; margin-bottom: 15px;">
    <b>Paso 1:</b> Selecciona un elemento del <b>Conjunto A</b> (se iluminará en azul).<br>
    <b>Paso 2:</b> Selecciona un elemento del <b>Conjunto B</b> para trazar la flecha de asignación.
  </p>

  <!-- Botones de Control -->
  <div style="text-align: center; margin-bottom: 15px; display: flex; justify-content: center; gap: 10px;">
    <button onclick="reiniciar()" style="padding: 6px 14px; background: #f1f5f9; border: 1px solid #cbd5e1; border-radius: 6px; color: #475569; font-weight: bold; cursor: pointer; font-size: 13px;">
      ↻ Borrar flechas
    </button>
  </div>

  <!-- Lienzo SVG con Diagramas de Venn y Flechas -->
  <svg id="lienzo-funcion" width="100%" height="340" viewBox="0 0 440 340" style="background: #f8fafc; border-radius: 8px; border: 1px solid #edf2f7;"></svg>
  ...
  <div style="margin-top: 15px; display: flex; flex-direction: column; gap: 10px;">
    ...
    <div style="background: #ffffff; border: 1px solid #e2e8f0; border-radius: 8px; padding: 10px; text-align: center;">
      <span style="font-size: 12px; font-weight: bold; text-transform: uppercase; color: #64748b; display: block; margin-bottom: 4px;">Pares ordenados asignados</span>
      <div id="texto-pares" style="font-family: 'Courier New', monospace; font-size: 15px; font-weight: bold; color: #0f172a;">
        f = { }
      </div>
    </div>

    <!-- Veredicto de Sobreyectividad -->
    <div id="panel-veredicto" style="padding: 12px; border-radius: 8px; text-align: center; font-weight: bold; font-size: 14px; background: #fef3c7; color: #92400e; border: 1px solid #fde68a;">
      Asigna flechas salientes para los 4 elementos del conjunto A.
    </div>
...
// Coordenadas Y para la distribución vertical
const posA = { a: 80, b: 140, c: 200, d: 260 };
const posB = { x: 80, y: 140, z: 200, w: 260 };

let elementoSeleccionadoA = null;
let asignacion = { a: null, b: null, c: null, d: null };
...
function renderizar() {
  ...
  let html = `
    ...
    <!-- Conjunto A (Dominio) -->
    <ellipse cx="110" cy="170" rx="65" ry="135" fill="#ebf8ff" stroke="#3182ce" stroke-width="2.5" stroke-dasharray="6 4"/>
    <text x="110" y="24" text-anchor="middle" font-weight="bold" font-size="18" fill="#2b6cb0">Conjunto A</text>

    <!-- Conjunto B (Codominio) -->
    <ellipse cx="330" cy="170" rx="65" ry="135" fill="#fffaf0" stroke="#dd6b20" stroke-width="2.5" stroke-dasharray="6 4"/>
    <text x="330" y="24" text-anchor="middle" font-weight="bold" font-size="18" fill="#c05621">Conjunto B</text>
  `;

  // Trazado de flechas activas
  for (const [origen, destino] of Object.entries(asignacion)) {
    if (destino !== null) {
      html += `<line x1="110" y1="${posA[origen]}" x2="330" y2="${posB[destino]}" stroke="#2563eb" stroke-width="2.5" marker-end="url(#punta-flecha)"/>`;
    }
  }

  // Nodos del Conjunto A
  domA.forEach(nodo => {
    const esActivo = (elementoSeleccionadoA === nodo);
    const bg = esActivo ? "#bfdbfe" : "#ffffff";
    const border = esActivo ? "#1d4ed8" : "#3b82f6";
    const width = esActivo ? "3.5" : "2";

    html += `<circle cx="110" cy="${posA[nodo]}" r="16" fill="${bg}" stroke="${border}" stroke-width="${width}" style="cursor: pointer;" onclick="seleccionarA('${nodo}')"/>`;
    html += `<text x="110" y="${posA[nodo]+5}" text-anchor="middle" font-weight="bold" font-size="15" fill="#1e40af" style="pointer-events: none;">${nodo}</text>`;
  });

  // Nodos del Conjunto B
  codB.forEach(nodo => {
    html += `<circle cx="330" cy="${posB[nodo]}" r="16" fill="#ffffff" stroke="#d97706" stroke-width="2" style="cursor: pointer;" onclick="asignarB('${nodo}')"/>`;
    html += `<text x="330" y="${posB[nodo]+5}" text-anchor="middle" font-weight="bold" font-size="15" fill="#92400e" style="pointer-events: none;">${nodo}</text>`;
  });

  svg.innerHTML = html;
  evaluarSobreyectividad();
}

function seleccionarA(nodo) {
  elementoSeleccionadoA = (elementoSeleccionadoA === nodo) ? null : nodo;
  renderizar();
}

function asignarB(nodo) {
  if (elementoSeleccionadoA !== null) {
    asignacion[elementoSeleccionadoA] = nodo;
    elementoSeleccionadoA = null;
    renderizar();
  }
}

function reiniciar() {
  asignacion = { a: null, b: null, c: null, d: null };
  elementoSeleccionadoA = null;
  renderizar();
}

function evaluarSobreyectividad() {
  const txtPares = document.getElementById('texto-pares');
  const panelVeredicto = document.getElementById('panel-veredicto');

  const pares = Object.entries(asignacion).filter(([_, dest]) => dest !== null);

  // Muestra de pares ordenados
  if (pares.length === 0) {
    txtPares.innerHTML = 'f = { }';
  } else {
    txtPares.innerHTML = `f = { ${pares.map(([orig, dest]) => `(${orig},${dest})`).join(', ')} }`;
  }

  // Verificación de totalidad (función bien definida)
  if (pares.length < 4) {
    const faltantes = 4 - pares.length;
    panelVeredicto.style.cssText = "padding: 12px; border-radius: 8px; text-align: center; font-weight: bold; font-size: 14px; background: #fef3c7; color: #92400e; border: 1px solid #fde68a;";
    panelVeredicto.innerHTML = `⚠️ Asignación incompleta: faltan ${faltantes} elemento(s) por conectar desde A.`;
    return;
  }

  // Cálculo de la Imagen Im(f)
  const conjuntoImagen = new Set(Object.values(asignacion));
  const noCubiertos = codB.filter(elem => !conjuntoImagen.has(elem));

  if (conjuntoImagen.size === 4) {
    panelVeredicto.style.cssText = "padding: 12px; border-radius: 8px; text-align: center; font-weight: bold; font-size: 14px; background: #dcfce7; color: #166534; border: 1px solid #86efac;";
    panelVeredicto.innerHTML = "✅ ES SOBREYECTIVA: Todos los elementos del codominio B reciben al menos una flecha (Im(f) = B).";
  } else {
    panelVeredicto.style.cssText = "padding: 12px; border-radius: 8px; text-align: center; font-weight: bold; font-size: 14px; background: #fee2e2; color: #991b1b; border: 1px solid #fca5a5;";
    panelVeredicto.innerHTML = `❌ NO ES SOBREYECTIVA: Hay elemento(s) en B sin preimagen: { ${noCubiertos.join(', ')} }.`;
  }
}
