...
```{=html}
<div style="font-family: sans-serif; max-width: 520px; margin: 10px auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 8px; background: #fff; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
  <p style="text-align: center; font-size: 15px; color: #4a5568; margin-top: 0;">
    Haz clic en un círculo del <b>Conjunto A</b> y luego en uno del <b>Conjunto B</b> para trazar la flecha.
  </p>
  
  <div style="text-align: center; margin-bottom: 15px;">
    <button onclick="window.reiniciar()" style="padding: 6px 16px; cursor: pointer; border: 1px solid #cbd5e0; background: #edf2f7; border-radius: 6px; font-weight: bold; color: #4a5568;">↻ Borrar flechas</button>
  </div>

  <!-- Contenedor SVG con dimensiones estrictas para prevenir colapsos en preview -->
  <div style="width: 100%; min-height: 340px; position: relative;">
    <svg id="lienzo-funcion" width="100%" height="340" viewBox="0 0 400 340" style="background: #f8fafc; border-radius: 6px; border: 1px solid #edf2f7; display: block;"></svg>
  </div>

  <!-- Muestra de la función en pares ordenados -->
  <div id="texto-conjunto" style="text-align: center; font-size: 16px; font-family: 'Courier New', monospace; font-weight: bold; margin-top: 15px; padding: 10px; background: #edf2f7; border-radius: 6px; color: #2d3748;">
    f = { }
  </div>

  <!-- Panel de Diagnóstico de Sobreyectividad -->
  <div id="panel-sobreyectiva" style="margin-top: 10px; padding: 12px; border-radius: 6px; text-align: center; font-weight: bold; font-size: 14px; background: #fef3c7; color: #92400e; border: 1px solid #fde68a;">
...
(function() {
  ...
  function dibujarSVG() {
    ...
    elementosA.forEach(nodo => {
      const seleccionado = (selA === nodo);
      const relleno = seleccionado ? "#bee3f8" : "#ffffff";
      const grosor = seleccionado ? "3" : "2";
      html += `<circle cx="100" cy="${yA[nodo]}" r="18" fill="${relleno}" stroke="#3182ce" stroke-width="${grosor}" style="cursor: pointer; transition: 0.2s;" onclick="window.clicEnA('${nodo}')"/>`;
      html += `<text x="100" y="${yA[nodo]+5}" text-anchor="middle" font-weight="bold" fill="#2b6cb0" style="pointer-events: none;">${nodo}</text>`;
    });
    ...
    elementosB.forEach(nodo => {
      const seleccionado = (selB === nodo);
      const relleno = seleccionado ? "#fbd38d" : "#ffffff";
      const grosor = seleccionado ? "3" : "2";
      html += `<circle cx="300" cy="${yB[nodo]}" r="18" fill="${relleno}" stroke="#dd6b20" stroke-width="${grosor}" style="cursor: pointer; transition: 0.2s;" onclick="window.clicEnB('${nodo}')"/>`;
      html += `<text x="300" y="${yB[nodo]+5}" text-anchor="middle" font-weight="bold" fill="#c05621" style="pointer-events: none;">${nodo}</text>`;
    });
  ...
  function actualizarEstado() {
    const contenedorTxt = document.getElementById('texto-conjunto');
    const panelSob = document.getElementById('panel-sobreyectiva');
    if (!contenedorTxt || !panelSob) return;
    ...
    if (parejas.length === 0) {
      contenedorTxt.innerHTML = 'f = { }';
    } else {
      contenedorTxt.innerHTML = `f = { ${parejas.join(', ')} }`;
    }

    const cantidadAsignada = Object.keys(funcion).length;

    if (cantidadAsignada < elementosA.length) {
      const faltantes = elementosA.length - cantidadAsignada;
      ...
      panelSob.innerHTML = `⚠️ Función incompleta: asigna los ${faltantes} elemento(s) faltantes de A.`;
      return;
    }

    const imagenes = new Set(Object.values(funcion));
    const noAlcanzados = elementosB.filter(b => !imagenes.has(b));

    if (imagenes.size === elementosB.length) {
  ...
  // Inicialización directa y resistente al Live Reload de Quarto Preview
  ...
  // Respaldo de renderizado diferido por si la animación de Quarto reestructura el DOM
  setTimeout(dibujarSVG, 50);
})();
</script>
```
