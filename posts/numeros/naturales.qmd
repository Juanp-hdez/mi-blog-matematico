...
```{=html}
<div style="font-family: system-ui, sans-serif; max-width: 520px; margin: 15px auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 10px; background: #ffffff; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
  
  <p style="text-align: center; font-size: 14px; color: #475569; margin-top: 0; margin-bottom: 15px;">
    Selecciona en cada casilla de <b>A</b> el elemento correspondiente de <b>B</b>:
  </p>

  <!-- Grilla de las 4 casillas (a, b, c, d) -->
  <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 10px; margin-bottom: 20px;">
    
    <!-- Casilla a -->
    <div style="border: 2px solid #3182ce; border-radius: 8px; padding: 8px; text-align: center; background: #ebf8ff;">
      <label style="font-weight: bold; color: #2b6cb0; display: block; margin-bottom: 4px;">Casilla a</label>
      <select id="casilla-a" onchange="dibujarDiagrama()" style="width: 100%; padding: 4px; border-radius: 4px; border: 1px solid #cbd5e1; font-weight: bold; text-align: center; cursor: pointer;">
    ...
    <!-- Casilla b -->
    <div style="border: 2px solid #3182ce; border-radius: 8px; padding: 8px; text-align: center; background: #ebf8ff;">
      <label style="font-weight: bold; color: #2b6cb0; display: block; margin-bottom: 4px;">Casilla b</label>
      <select id="casilla-b" onchange="dibujarDiagrama()" style="width: 100%; padding: 4px; border-radius: 4px; border: 1px solid #cbd5e1; font-weight: bold; text-align: center; cursor: pointer;">
        <option value="x">x</option>
    ...
    <!-- Casilla c -->
    <div style="border: 2px solid #3182ce; border-radius: 8px; padding: 8px; text-align: center; background: #ebf8ff;">
      <label style="font-weight: bold; color: #2b6cb0; display: block; margin-bottom: 4px;">Casilla c</label>
      <select id="casilla-c" onchange="dibujarDiagrama()" style="width: 100%; padding: 4px; border-radius: 4px; border: 1px solid #cbd5e1; font-weight: bold; text-align: center; cursor: pointer;">
        <option value="x">x</option>
        <option value="y">y</option>
    ...
    <!-- Casilla d -->
    <div style="border: 2px solid #3182ce; border-radius: 8px; padding: 8px; text-align: center; background: #ebf8ff;">
      <label style="font-weight: bold; color: #2b6cb0; display: block; margin-bottom: 4px;">Casilla d</label>
      <select id="casilla-d" onchange="dibujarDiagrama()" style="width: 100%; padding: 4px; border-radius: 4px; border: 1px solid #cbd5e1; font-weight: bold; text-align: center; cursor: pointer;">
        <option value="x">x</option>
        <option value="y">y</option>
        <option value="z">z</option>
  ...
  <!-- Lienzo donde se renderiza el diagrama gráfico -->
  <svg id="lienzo-interactivo" width="100%" height="280" viewBox="0 0 400 280" style="background: #f8fafc; border-radius: 8px; border: 1px solid #edf2f7;"></svg>

  <!-- Notación de conjunto matemática -->
  <div id="resultado-funcion" style="text-align: center; font-family: 'Courier New', monospace; font-size: 16px; font-weight: bold; margin-top: 15px; padding: 10px; background: #f1f5f9; border-radius: 6px; color: #1e293b;">
    f = { }
  </div>
...
function dibujarDiagrama() {
  const svg = document.getElementById('lienzo-interactivo');
  const txtMatematico = document.getElementById('resultado-funcion');
  if (!svg) return;

  // Valores leídos de cada casilla
  const valA = document.getElementById('casilla-a').value;
  const valB = document.getElementById('casilla-b').value;
  const valC = document.getElementById('casilla-c').value;
  const valD = document.getElementById('casilla-d').value;

  // Posiciones en el espacio SVG
  const posA = { a: 45, b: 110, c: 175, d: 240 };
  const posB = { x: 45, y: 110, z: 175, w: 240 };
  ...
  let html = `
    <defs>
      <marker id="punta-flecha-casilla" viewBox="0 0 10 10" refX="28" refY="5" markerWidth="6" markerHeight="6" orient="auto">
        <path d="M 0 0 L 10 5 L 0 10 z" fill="#2b6cb0"/>
      </marker>
    </defs>
    <text x="100" y="20" text-anchor="middle" font-weight="bold" fill="#334155">Conjunto A</text>
    <text x="300" y="20" text-anchor="middle" font-weight="bold" fill="#334155">Conjunto B</text>
  `;

  // Trazado de flechas
  parejas.forEach(p => {
    const y1 = posA[p.orig];
    const y2 = posB[p.dest];
    html += `<line x1="100" y1="${y1}" x2="300" y2="${y2}" stroke="#2b6cb0" stroke-width="2.5" marker-end="url(#punta-flecha-casilla)"/>`;
  });

  // Nodos del Conjunto A
  Object.keys(posA).forEach(key => {
    const y = posA[key];
    html += `<circle cx="100" cy="${y}" r="16" fill="#ebf8ff" stroke="#3182ce" stroke-width="2"/>`;
    html += `<text x="100" y="${y+5}" text-anchor="middle" font-weight="bold" fill="#2b6cb0">${key}</text>`;
  });

  // Nodos del Conjunto B
  Object.keys(posB).forEach(key => {
    const y = posB[key];
    html += `<circle cx="300" cy="${y}" r="16" fill="#feebc8" stroke="#dd6b20" stroke-width="2"/>`;
    html += `<text x="300" y="${y+5}" text-anchor="middle" font-weight="bold" fill="#c05621">${key}</text>`;
  });

  svg.innerHTML = html;

  // Actualizar representación en notación de conjuntos
  const strParejas = parejas.map(p => `(${p.orig}, ${p.dest})`).join(', ');
  txtMatematico.innerHTML = `f = { ${strParejas} }`;
}

// Renderizado inicial
document.addEventListener("DOMContentLoaded", dibujarDiagrama);
dibujarDiagrama();
</script>
´´´