...
```{=html}
<div id="app-inyectiva-reversa" style="font-family: system-ui, -apple-system, sans-serif; max-width: 850px; margin: 10px auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 10px; background: #fff; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
  <h4 style="text-align: center; color: #1e293b; margin-top: 0; margin-bottom: 5px;">Función Inyectiva y Restricción del Codominio</h4>
  <p style="text-align: center; font-size: 14px; color: #64748b; margin-bottom: 15px;">
    Define la función $f: A \to B$ en el <b>Gráfico 1</b> (haz clic en un elemento de A y luego en uno de B). Si la función es <b>inyectiva</b>, el <b>Gráfico 2</b> eliminará automáticamente los elementos de $B$ no asignados y revertirá las flechas para obtener la función inversa $f^{-1}: \text{Im}(f) \to A$.
  </p>

  <div style="text-align: center; margin-bottom: 15px;">
    <button id="btn-reiniciar-inversa" style="padding: 6px 16px; cursor: pointer; border: 1px solid #cbd5e0; background: #edf2f7; border-radius: 6px; font-weight: bold; color: #4a5568;">↻ Restablecer asignación por defecto</button>
  </div>
  ...
  <div style="display: flex; flex-wrap: wrap; gap: 20px; justify-content: center;">
    ...
    <div style="flex: 1; min-width: 360px; max-width: 400px; background: #f8fafc; border: 1px solid #edf2f7; border-radius: 8px; padding: 10px; text-align: center;">
      ...
      <svg id="lienzo-directa-rev" width="100%" height="320" viewBox="0 0 400 320" style="touch-action: manipulation;"></svg>
      
      <div id="txt-directa-rev" style="font-family: 'Courier New', monospace; font-size: 14px; font-weight: bold; margin-top: 10px; padding: 8px; background: #edf2f7; border-radius: 6px; color: #2d3748;">
        f = { (a, x), (b, y), (c, z) }
      </div>
      <div id="panel-directa-rev" style="margin-top: 8px; padding: 8px; border-radius: 6px; font-weight: bold; font-size: 13px; background: #dcfce7; color: #166534; border: 1px solid #86efac;">
        ✅ ES INYECTIVA
      </div>
    </div>

    <!-- GRÁFICO 2: Inversa sobre Im(f) con B filtrado -->
    <div style="flex: 1; min-width: 360px; max-width: 400px; background: #f8fafc; border: 1px solid #edf2f7; border-radius: 8px; padding: 10px; text-align: center;">
      <div style="font-weight: bold; color: #c05621; margin-bottom: 8px;">2. Inversa $f^{-1}: \text{Im}(f) \to A$ (Sin elementos no asignados)</div>
      <svg id="lienzo-reversa-filtrado" width="100%" height="320" viewBox="0 0 400 320"></svg>
      
      <div id="txt-reversa-filtrado" style="font-family: 'Courier New', monospace; font-size: 14px; font-weight: bold; margin-top: 10px; padding: 8px; background: #edf2f7; border-radius: 6px; color: #2d3748;">
        f⁻¹ = { (x, a), (y, b), (z, c) }
      </div>
      <div id="panel-reversa-filtrado" style="margin-top: 8px; padding: 8px; border-radius: 6px; font-weight: bold; font-size: 13px; background: #dcfce7; color: #166534; border: 1px solid #86efac;">
        ✅ Inversa válida (Biyecion sobre Im(f))
      </div>
    </div>
...
<script>
(function() {
  function iniciarModuloReversa() {
    const root = document.getElementById('app-inyectiva-reversa');
    if (!root) return;

    const svgDirecta = root.querySelector('#lienzo-directa-rev');
    const svgReversa = root.querySelector('#lienzo-reversa-filtrado');
    const txtDirecta = root.querySelector('#txt-directa-rev');
    const txtReversa = root.querySelector('#txt-reversa-filtrado');
    const panelDirecta = root.querySelector('#panel-directa-rev');
    const panelReversa = root.querySelector('#panel-reversa-filtrado');
    const btnReiniciar = root.querySelector('#btn-reiniciar-inversa');
    ...
    // Estado inicial inyectivo
    let funcion = { a: 'x', b: 'y', c: 'z' };

    function renderDirecta() {
      let html = `
        <defs>
          <marker id="punta-dir-rev" viewBox="0 0 10 10" refX="28" refY="5" markerWidth="6" markerHeight="6" orient="auto">
            <path d="M 0 0 L 10 5 L 0 10 z" fill="#2b6cb0"/>
          </marker>
        </defs>
      ...
      for (const [a, b] of Object.entries(funcion)) {
        if (b) {
          html += `<line x1="100" y1="${yA[a]}" x2="300" y2="${yB[b]}" stroke="#2b6cb0" stroke-width="2.5" marker-end="url(#punta-dir-rev)"/>`;
        }
      }
    ...
    function renderReversa(esInyectiva, imF) {
      if (!esInyectiva || Object.keys(funcion).length < elemA.length) {
        svgReversa.innerHTML = `
          <text x="200" y="150" text-anchor="middle" fill="#94a3b8" font-size="14" font-weight="bold">
            No se puede generar la inversa:
          </text>
          <text x="200" y="175" text-anchor="middle" fill="#94a3b8" font-size="13">
      ...
      // Ordenar los elementos de Im(f) manteniendo el orden original
      ...
      let html = `
        <defs>
          <marker id="punta-rev-filtrado" viewBox="0 0 10 10" refX="28" refY="5" markerWidth="6" markerHeight="6" orient="auto">
            <path d="M 0 0 L 10 5 L 0 10 z" fill="#c05621"/>
          </marker>
        </defs>

        <!-- Conjunto Im(f) (3 elementos) -->
      ...
      // Flechas Revertidas Im(f) -> A
      const parejasInv = [];
      for (const [a, b] of Object.entries(funcion)) {
        parejasInv.push(`(${b}, ${a})`);
        html += `<line x1="100" y1="${yB_Filtrado[b]}" x2="300" y2="${yA[a]}" stroke="#c05621" stroke-width="2.5" marker-end="url(#punta-rev-filtrado)"/>`;
      }
      ...
      bFiltrado.forEach(nodo => {
        html += `<g>
          ...
          <text x="100" y="${yB_Filtrado[nodo]+5}" text-anchor="middle" font-weight="bold" fill="#c05621">${nodo}</text>
        </g>`;
      });
      ...
      elemA.forEach(nodo => {
        html += `<g>
          ...
          <text x="300" y="${yA[nodo]+5}" text-anchor="middle" font-weight="bold" fill="#2b6cb0">${nodo}</text>
        </g>`;
      });
      ...
      panelReversa.innerHTML = "✅ Inversa construida con éxito sobre Im(f).";
    }
    ...
    function actualizar() {
      ...
      if (esInyectiva) {
        const noAsignados = elemB.filter(b => !imF.includes(b));
        ...
        panelDirecta.innerHTML = `✅ ES INYECTIVA (Sin asignación en B: { ${noAsignados.join(', ')} })`;
      } else {
        const conteo = {};
        ...
        panelDirecta.innerHTML = `❌ NO ES INYECTIVA (Elemento colisionado: { ${repetidos.join(', ')} })`;
      }
  ...
  if (document.readyState === 'complete' || document.readyState === 'interactive') {
    setTimeout(iniciarModuloReversa, 10);
  } else {
    document.addEventListener('DOMContentLoaded', iniciarModuloReversa);
  }
})();
</script>
```