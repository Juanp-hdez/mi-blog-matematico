...
```{=html}
<div id="app-composicion-aleatoria-v2" style="font-family: system-ui, -apple-system, sans-serif; max-width: 880px; margin: 15px auto; padding: 22px; border: 1px solid #e2e8f0; border-radius: 12px; background: #ffffff; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">

  <h3 style="text-align: center; color: #1e293b; margin-top: 0; margin-bottom: 6px;">Composición de Funciones</h3>
  <p style="text-align: center; font-size: 14px; color: #64748b; margin-bottom: 18px;">
    Dadas las funciones f y g, determine cómo queda definida la función compuesta g ∘ f.
  </p>

  <!-- Botón Generador -->
  <div style="text-align: center; margin-bottom: 20px;">
    <button id="btn-generar-aleatorias-v2" style="padding: 8px 18px; cursor: pointer; border: 1px solid #2563eb; background: #3b82f6; color: white; border-radius: 6px; font-weight: bold; font-size: 14px; box-shadow: 0 2px 4px rgba(37,99,235,0.2);">
      🎲 Generar Nuevas Funciones f y g
    </button>
  </div>

  <!-- DIAGRAMA DE LAS FUNCIONES GENERADAS f y g CON FLECHAS SUPERIORES -->
  <div style="background: #f8fafc; border: 1px solid #edf2f7; border-radius: 10px; padding: 15px; margin-bottom: 25px; text-align: center;">
    <div style="font-weight: bold; color: #334155; margin-bottom: 10px; font-size: 15px;">
      1. Funciones Dadas: f y g
    </div>
    
    <div style="position: relative; width: 100%; min-height: 300px; overflow-x: auto;">
      <svg id="svg-funciones-dadas-v2" width="100%" height="300" viewBox="0 0 620 300" style="display: block; margin: 0 auto;"></svg>
    </div>

    <!-- Muestra analítica de f y g -->
    <div style="display: flex; gap: 12px; justify-content: center; flex-wrap: wrap; margin-top: 10px;">
      <div id="txt-f-dada-v2" style="font-family: 'Courier New', monospace; font-size: 13px; font-weight: bold; padding: 6px 12px; background: #eff6ff; border: 1px solid #bfdbfe; border-radius: 6px; color: #1e40af;">
        f = { }
      </div>
      <div id="txt-g-dada-v2" style="font-family: 'Courier New', monospace; font-size: 13px; font-weight: bold; padding: 6px 12px; background: #fff7ed; border: 1px solid #fed7aa; border-radius: 6px; color: #9a3412;">
        g = { }
      </div>
    </div>
  </div>
  ...
  <div style="background: #f8fafc; border: 1px solid #edf2f7; border-radius: 10px; padding: 18px; text-align: center;">
    <div style="font-weight: bold; color: #6b21a8; margin-bottom: 6px; font-size: 16px;">
      2. Construye la compuesta de g con f 
    

    <!-- Indicador de acción -->
    <div id="guia-usuario-v2" style="font-size: 13px; font-weight: bold; padding: 6px 12px; border-radius: 6px; background: #f1f5f9; color: #475569; margin: 0 auto 15px auto; max-width: 450px;">
      Selecciona un elemento del Conjunto A y luego uno del conjunto C para definir su imagen.
    </div>

    <div style="position: relative; width: 100%; min-height: 290px;">
      <svg id="svg-compuesta-interactiva-v2" width="100%" height="290" viewBox="0 0 420 290" style="display: block; margin: 0 auto; touch-action: manipulation;"></svg>
    </div>

    <!-- Expresión matemática del usuario -->
    <div id="txt-g-of-user-v2" style="font-family: 'Courier New', monospace; font-size: 15px; font-weight: bold; margin-top: 12px; padding: 8px; background: #ffffff; border: 1px solid #cbd5e1; border-radius: 6px; color: #0f172a; display: inline-block; min-width: 250px;">
      g ∘ f = { }
    </div>

    <!-- Panel de verificación de respuesta -->
    <div id="panel-verificacion-v2" style="margin-top: 12px; padding: 10px; border-radius: 6px; font-weight: bold; font-size: 14px; background: #fef3c7; color: #92400e; border: 1px solid #fde68a;">
...
<script>
(function() {
  function iniciarModuloComposicionV2() {
    const root = document.getElementById('app-composicion-aleatoria-v2');
    if (!root) return;

    const svgDadas = root.querySelector('#svg-funciones-dadas-v2');
    const svgCompuesta = root.querySelector('#svg-compuesta-interactiva-v2');
    const txtFdada = root.querySelector('#txt-f-dada-v2');
    const txtGdada = root.querySelector('#txt-g-dada-v2');
    const txtGofUser = root.querySelector('#txt-g-of-user-v2');
    const panelVerif = root.querySelector('#panel-verificacion-v2');
    const guiaUser = root.querySelector('#guia-usuario-v2');
    const btnGenerar = root.querySelector('#btn-generar-aleatorias-v2');
    ...
    // Coordenadas Y para lienzo de 300px
    const yA = { a: 110, b: 175, c: 240 };
    const yB = { x: 90, y: 145, z: 205, w: 260 };
    const yC = { t: 110, u: 175, v: 240 };
    ...
    // Dibujar Diagrama A -> B -> C con flechas superiores f y g
    function renderDadas() {
      let html = `
        <defs>
          <marker id="arrow-f-v2" viewBox="0 0 10 10" refX="26" refY="5" markerWidth="5" markerHeight="5" orient="auto">
            <path d="M 0 0 L 10 5 L 0 10 z" fill="#2563eb"/>
          </marker>
          <marker id="arrow-g-v2" viewBox="0 0 10 10" refX="26" refY="5" markerWidth="5" markerHeight="5" orient="auto">
            <path d="M 0 0 L 10 5 L 0 10 z" fill="#ea580c"/>
          </marker>
          <marker id="arrow-top-f" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="6" markerHeight="6" orient="auto">
            <path d="M 0 0 L 10 5 L 0 10 z" fill="#2563eb"/>
          </marker>
          <marker id="arrow-top-g" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="6" markerHeight="6" orient="auto">
            <path d="M 0 0 L 10 5 L 0 10 z" fill="#ea580c"/>
          </marker>
        </defs>

        <!-- FLECHAS SUPERIORES ENTRE CONJUNTOS (f y g) -->
        <path d="M 110,50 Q 200,15 290,50" fill="none" stroke="#2563eb" stroke-width="2.5" marker-end="url(#arrow-top-f)"/>
        <text x="200" y="24" text-anchor="middle" font-weight="bold" font-size="18" fill="#2563eb">f</text>

        <path d="M 330,50 Q 420,15 510,50" fill="none" stroke="#ea580c" stroke-width="2.5" marker-end="url(#arrow-top-g)"/>
        <text x="420" y="24" text-anchor="middle" font-weight="bold" font-size="18" fill="#ea580c">g</text>

        <!-- Óvalo A -->
        <ellipse cx="90" cy="175" rx="48" ry="95" fill="#ebf8ff" stroke="#3182ce" stroke-width="2" stroke-dasharray="5 4"/>
        <text x="90" y="66" text-anchor="middle" font-weight="bold" font-size="15" fill="#2b6cb0">A</text>

        <!-- Óvalo B -->
        <ellipse cx="310" cy="175" rx="48" ry="110" fill="#fffaf0" stroke="#dd6b20" stroke-width="2" stroke-dasharray="5 4"/>
        <text x="310" y="52" text-anchor="middle" font-weight="bold" font-size="15" fill="#c05621">B</text>

        <!-- Óvalo C -->
        <ellipse cx="530" cy="175" rx="48" ry="95" fill="#f0fdf4" stroke="#16a34a" stroke-width="2" stroke-dasharray="5 4"/>
        <text x="530" y="66" text-anchor="middle" font-weight="bold" font-size="15" fill="#15803d">C</text>
      `;

      // Flechas f: A -> B
      for (const [a, b] of Object.entries(f)) {
        html += `<line x1="90" y1="${yA[a]}" x2="310" y2="${yB[b]}" stroke="#2563eb" stroke-width="2" marker-end="url(#arrow-f-v2)"/>`;
      }

      // Flechas g: B -> C
      for (const [b, c] of Object.entries(g)) {
        html += `<line x1="310" y1="${yB[b]}" x2="530" y2="${yC[c]}" stroke="#ea580c" stroke-width="2" marker-end="url(#arrow-g-v2)"/>`;
      }
    ...
    // Dibujar Diagrama Interactivo A -> C con flecha superior g o f
    function renderCompuesta() {
      let html = `
        <defs>
          <marker id="arrow-gof-v2" viewBox="0 0 10 10" refX="28" refY="5" markerWidth="6" markerHeight="6" orient="auto">
            <path d="M 0 0 L 10 5 L 0 10 z" fill="#9333ea"/>
          </marker>
          <marker id="arrow-top-gof" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="6" markerHeight="6" orient="auto">
            <path d="M 0 0 L 10 5 L 0 10 z" fill="#9333ea"/>
          </marker>
        </defs>

        <!-- FLECHA SUPERIOR g o f -->
        <path d="M 120,50 Q 210,15 300,50" fill="none" stroke="#9333ea" stroke-width="2.5" stroke-dasharray="5 4" marker-end="url(#arrow-top-gof)"/>
        <text x="210" y="24" text-anchor="middle" font-weight="bold" font-size="18" fill="#9333ea">g ∘ f</text>

        <!-- Óvalo A -->
        <ellipse cx="100" cy="175" rx="55" ry="95" fill="#ebf8ff" stroke="#3182ce" stroke-width="2" stroke-dasharray="5 4"/>
        <text x="100" y="66" text-anchor="middle" font-weight="bold" font-size="15" fill="#2b6cb0">A</text>

        <!-- Óvalo C -->
        <ellipse cx="320" cy="175" rx="55" ry="95" fill="#f0fdf4" stroke="#16a34a" stroke-width="2" stroke-dasharray="5 4"/>
        <text x="320" y="66" text-anchor="middle" font-weight="bold" font-size="15" fill="#15803d">C</text>
      `;

      // Flechas g o f (Usuario)
      for (const [a, c] of Object.entries(userGof)) {
        if (c) {
          html += `<line x1="100" y1="${yA[a]}" x2="320" y2="${yC[c]}" stroke="#9333ea" stroke-width="2.5" marker-end="url(#arrow-gof-v2)"/>`;
        }
      }
    ...
    function evaluarRespuesta() {
      if (selA) {
        ...
        guiaUser.innerHTML = `Haz clic en un elemento del Conjunto A para luego asignar su imagen.`;
      }
  ...
  if (document.readyState === 'complete' || document.readyState === 'interactive') {
    setTimeout(iniciarModuloComposicionV2, 50);
  } else {
    document.addEventListener('DOMContentLoaded', iniciarModuloComposicionV2);
  }
})();
</script>
```
...
<div id="app-inyectiva-reversa-v2" style="font-family: system-ui, -apple-system, sans-serif; max-width: 860px; margin: 15px auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 10px; background: #ffffff; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
  ...
  <p style="text-align: center; font-size: 14px; color: #64748b; margin-bottom: 15px;">
    <b>Paso 1:</b> Haz clic en un elemento del <b>A</b>. <br>
    <b>Paso 2:</b> Haz clic en un elemento del <b>B</b> para asignarle su imagen.
  </p>
  ...
  <div id="guia-pasos" style="text-align: center; font-size: 13px; font-weight: bold; padding: 8px; border-radius: 6px; background: #f1f5f9; color: #334155; margin-bottom: 15px;">
    Haz clic en un elemento del Conjunto A para comenzar.
  </div>
