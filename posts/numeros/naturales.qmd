...
```{=html}
<div id="app-inversa-v1" style="font-family: system-ui, -apple-system, sans-serif; max-width: 850px; margin: 10px auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 10px; background: #fff; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
  <h4 style="text-align: center; color: #1e293b; margin-top: 0; margin-bottom: 5px;">Función Inyectiva y Inversión de Flechas</h4>
  <p style="text-align: center; font-size: 14px; color: #64748b; margin-bottom: 15px;">
    Modifica la función $f: A \to B$ en el <b>Gráfico 1</b> (clic en A y luego en B). El <b>Gráfico 2</b> proyectará automáticamente las <b>flechas revertidas</b> de la relación.
  </p>

  <div style="text-align: center; margin-bottom: 15px;">
    <button id="btn-reiniciar-inv" style="padding: 6px 16px; cursor: pointer; border: 1px solid #cbd5e0; background: #edf2f7; border-radius: 6px; font-weight: bold; color: #4a5568;">↻ Restablecer asignación inyectiva por defecto</button>
  </div>

  <div style="display: flex; flex-wrap: wrap; gap: 20px; justify-content: center;">
    
    <!-- GRÁFICO 1: Función f: A -> B -->
    <div style="flex: 1; min-width: 360px; max-width: 400px; background: #f8fafc; border: 1px solid #edf2f7; border-radius: 8px; padding: 10px; text-align: center;">
      <div style="font-weight: bold; color: #2b6cb0; margin-bottom: 8px;">1. Función Directa $f: A \to B$</div>
      <svg id="lienzo-f-directa" width="100%" height="320" viewBox="0 0 400 320" style="touch-action: manipulation;"></svg>
      
      <div id="txt-f-directa" style="font-family: 'Courier New', monospace; font-size: 14px; font-weight: bold; margin-top: 10px; padding: 8px; background: #edf2f7; border-radius: 6px; color: #2d3748;">
        f = { (a, x), (b, y), (c, z) }
      </div>
      <div id="panel-f-directa" style="margin-top: 8px; padding: 8px; border-radius: 6px; font-weight: bold; font-size: 13px; background: #dcfce7; color: #166534; border: 1px solid #86efac;">
        ✅ ES INYECTIVA
      </div>
    </div>

    <!-- GRÁFICO 2: Relación Inversa (Flechas Revertidas) -->
    <div style="flex: 1; min-width: 360px; max-width: 400px; background: #f8fafc; border: 1px solid #edf2f7; border-radius: 8px; padding: 10px; text-align: center;">
      <div style="font-weight: bold; color: #c05621; margin-bottom: 8px;">2. Relación Inversa (Flechas Revertidas)</div>
      <svg id="lienzo-f-inversa" width="100%" height="320" viewBox="0 0 400 320"></svg>
      
      <div id="txt-f-inversa" style="font-family: 'Courier New', monospace; font-size: 14px; font-weight: bold; margin-top: 10px; padding: 8px; background: #edf2f7; border-radius: 6px; color: #2d3748;">
        R⁻¹ = { (x, a), (y, b), (z, c) }
      </div>
      <div id="panel-f-inversa" style="margin-top: 8px; padding: 8px; border-radius: 6px; font-weight: bold; font-size: 13px; background: #eff6ff; color: #1e40af; border: 1px solid #bfdbfe;">
        ℹ️ Función parcial en B (w no tiene salida).
      </div>
    </div>
...
<script>
(function() {
  function iniciarModuloInversa() {
    const root = document.getElementById('app-inversa-v1');
    if (!root) return;

    const svgDirecta = root.querySelector('#lienzo-f-directa');
    const svgInversa = root.querySelector('#lienzo-f-inversa');
    const txtDirecta = root.querySelector('#txt-f-directa');
    const txtInversa = root.querySelector('#txt-f-inversa');
    const panelDirecta = root.querySelector('#panel-f-directa');
    const panelInversa = root.querySelector('#panel-f-inversa');
    const btnReiniciar = root.querySelector('#btn-reiniciar-inv');

    if (!svgDirecta || !svgInversa) return;
    ...
    // Función inyectiva inicial
    let funcion = { a: 'x', b: 'y', c: 'z' };

    function renderDirecta() {
      let html = `
        <defs>
          <marker id="punta-dir" viewBox="0 0 10 10" refX="28" refY="5" markerWidth="6" markerHeight="6" orient="auto">
            <path d="M 0 0 L 10 5 L 0 10 z" fill="#2b6cb0"/>
          </marker>
        </defs>
      ...
      for (const [a, b] of Object.entries(funcion)) {
        if (b) {
          html += `<line x1="100" y1="${yA[a]}" x2="300" y2="${yB[b]}" stroke="#2b6cb0" stroke-width="2.5" marker-end="url(#punta-dir)"/>`;
        }
      }
    ...
    function renderInversa() {
      let html = `
        <defs>
          <marker id="punta-inv" viewBox="0 0 10 10" refX="28" refY="5" markerWidth="6" markerHeight="6" orient="auto">
      ...
      // Flechas Revertidas B -> A
      for (const [a, b] of Object.entries(funcion)) {
        if (b) {
          html += `<line x1="300" y1="${yB[b]}" x2="100" y2="${yA[a]}" stroke="#c05621" stroke-width="2.5" marker-end="url(#punta-inv)"/>`;
        }
      }
      ...
      elemA.forEach(nodo => {
        html += `<g>
          <circle cx="100" cy="${yA[nodo]}" r="18" fill="#ffffff" stroke="#3182ce" stroke-width="2"/>
          <text x="100" y="${yA[nodo]+5}" text-anchor="middle" font-weight="bold" fill="#2b6cb0">${nodo}</text>
        </g>`;
      });
      ...
      elemB.forEach(nodo => {
        html += `<g>
          <circle cx="300" cy="${yB[nodo]}" r="18" fill="#ffffff" stroke="#dd6b20" stroke-width="2"/>
          <text x="300" y="${yB[nodo]+5}" text-anchor="middle" font-weight="bold" fill="#c05621">${nodo}</text>
        </g>`;
      });

      svgInversa.innerHTML = html;
    }
    ...
    function actualizarEstados() {
      ...
      elemA.forEach(a => {
        if (funcion[a]) {
          parejasDir.push(`(${a}, ${funcion[a]})`);
          parejasInv.push(`(${funcion[a]}, ${a})`);
        }
      });

      txtDirecta.innerHTML = parejasDir.length ? `f = { ${parejasDir.join(', ')} }` : 'f = { }';
      txtInversa.innerHTML = parejasInv.length ? `R⁻¹ = { ${parejasInv.join(', ')} }` : 'R⁻¹ = { }';
      ...
      if (asignados < elemA.length) {
        ...
        panelDirecta.innerHTML = `⚠️ Asigna los ${elemA.length - asignados} elemento(s) faltantes.`;
      } else {
        const valoresB = Object.values(funcion);
        const unicos = new Set(valoresB);
        if (unicos.size === elemA.length) {
          ...
          panelDirecta.innerHTML = "✅ ES INYECTIVA";
        } else {
          ...
          panelDirecta.innerHTML = "❌ NO ES INYECTIVA (imágenes duplicadas)";
        }
      }

      const imagenes = new Set(Object.values(funcion));
      const noCubiertosB = elemB.filter(b => !imagenes.has(b));
      
      const valoresB = Object.values(funcion);
      const conteoB = {};
      valoresB.forEach(b => conteoB[b] = (conteoB[b] || 0) + 1);
      const conMasDeUnA = Object.keys(conteoB).filter(b => conteoB[b] > 1);

      if (asignados === elemA.length && noCubiertosB.length === 1 && conMasDeUnA.length === 0) {
        panelInversa.style.cssText = "margin-top: 8px; padding: 8px; border-radius: 6px; font-weight: bold; font-size: 13px; background: #eff6ff; color: #1e40af; border: 1px solid #bfdbfe;";
        panelInversa.innerHTML = `ℹ️ Función parcial en B: { ${noCubiertosB.join(', ')} } no tiene imagen inversa.`;
      } else if (conMasDeUnA.length > 0) {
        panelInversa.style.cssText = "margin-top: 8px; padding: 8px; border-radius: 6px; font-weight: bold; font-size: 13px; background: #fee2e2; color: #991b1b; border: 1px solid #fca5a5;";
        panelInversa.innerHTML = `❌ No es función: { ${conMasDeUnA.join(', ')} } tendría múltiples salidas.`;
      } else {
        panelInversa.style.cssText = "margin-top: 8px; padding: 8px; border-radius: 6px; font-weight: bold; font-size: 13px; background: #fef3c7; color: #92400e; border: 1px solid #fde68a;";
        panelInversa.innerHTML = `⚠️ Relación inversa definida sobre Im(f).`;
      }
    }
    ...
    svgDirecta.addEventListener('click', (e) => {
      const targetGroup = e.target.closest('[data-grupo]');
      if (!targetGroup) return;
  ...
  if (document.readyState === 'complete' || document.readyState === 'interactive') {
    setTimeout(iniciarModuloInversa, 10);
  } else {
    document.addEventListener('DOMContentLoaded', iniciarModuloInversa);
  }
})();
</script>
```